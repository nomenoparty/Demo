...
//	string day; // ngày, lưu ngày-tháng-năm
//	string hour; // giờ, lưu giờ-phút
...
int createFilmsWindow(sf::RenderWindow& window, sf::RenderWindow& window1, sf::Font& font, float filmListPosition1)
{
	...
	sf::RectangleShape buttonDuration(sf::Vector2f(50, 45));
...
void readFromFile() {
	std::ifstream inputFile("DanhSachPhim1.txt");
	if (inputFile.is_open()) {
		...
		while (getline(inputFile, line)) {
			...
			films.push_back({ temp[0], temp[1], temp[2] });
		}
	}
	else {
...
void writeToFile() {
	...
	for (int i = 0; i < films.size(); i++) {
		fileOut << films[i].title << "$" << films[i].desc << "$" << films[i].image << endl;
	}
	fileOut.close();
}
			...
			if (countPage == 1) {
				...
				if (!texturePhimPage6.loadFromFile(selectedMovie + ".jpg")) {
					if (!texturePhimPage6.loadFromFile(selectedMovie + ".png")) {
						if (!texturePhimPage6.loadFromFile(selectedMovie + ".jfif")) {
							std::cerr << "Error loading movieSpriteTexture texture!" << std::endl;
							return -1;
						}
					}
...
//	sf::Sprite clockSprite(clockTexture);
//	clockSprite.setPosition(0, 5);
//	float scalex = 158.0 / clockTexture.getSize().x;
//	float scaley = 100.0 / clockTexture.getSize().y;
//	clockSprite.setScale(scalex, scaley);
//
//	// Tạo văn bản sử dụng font đã tải
//	//sf::Text textClockDay("", fontClock, 24);
